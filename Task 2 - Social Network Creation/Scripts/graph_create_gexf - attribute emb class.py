...
with open(embeddingPath, "r", encoding="utf-8") as embeddingFile:
	...
	for embeddingNum, embedding in enumerate(embeddingFile):
		...
		if (xVal < -2 and yVal < -1): 
			embeddingClassDict[nodeID] = "ball"
		else:
			embeddingClassDict[nodeID] = "boomerang" 
...
with open("../Data/Genius Info - Billboard + MSD, 1990-2010.json", "r", encoding="utf-8") as datasetJson:
	...
	for songNum, songDict in enumerate(songList):
		...
		artistID_OG = artistID
		artistID = str(artistID) + " (" + artistName + ")"
		if (not artistID_OG in artistDict):
			songClass = "none"
			if (int(artistID_OG) in embeddingClassDict):
				songClass = embeddingClassDict[int(artistID_OG)]
			artistDict[artistID] = {"name": artistName, "ID": artistID, "songs": [], "collaborators": [], "types": [], "class": songClass}
		...
		for collaborator in songDict["collaborators"]:
			...
			collaboratorID_OG = collaboratorID
			collaboratorID = str(collaboratorID) + " (" + collaboratorName + ")"
			if (not collaboratorID in artistDict):
				songClass = "none"
				if (int(collaboratorID_OG) in embeddingClassDict):
					songClass = embeddingClassDict[int(collaboratorID_OG)]
				artistDict[collaboratorID] = {"name": collaboratorName, "ID": collaboratorID, "songs": [], "collaborators": [], "types": [], "class": songClass}
			...
			if (MORE_CONNECTIONS_SMART):
				...
				for otherColab in songDict["collaborators"]:
					...
					if (not otherColabID in artistDict):
						songClass = "none"
						if (int(otherColabID_OG) in embeddingClassDict):
							songClass = embeddingClassDict[int(otherColabID_OG)]
						artistDict[otherColabID] = {"name": otherColabName, "ID": otherColabID, "songs": [], "collaborators": [], "types": [], "class": songClass}
...
for artistNum, artistID in enumerate(list(artistDict.keys())):
	...
	nodeClass = curArtistDict["class"]

	print("\n%d / %d: Adding %s to the graph..." % (artistNum, numArtists, artistName))

	# Add the node for the current artist
	graph.add_node(artistID, name=artistName, types=str(typeList), maxType=maxType, node2vec_class=nodeClass)
	...
	for collaborator in collaboratorList:
		...
		nodeClass = artistDict[collaboratorID]["class"]
		if (not graph.has_node(collaboratorID)):
			graph.add_node(collaboratorID, name=collaboratorName, types=str(collaboratorTypes), maxType=collaboratorMaxType, node2vec_class=nodeClass)
		...
		if (MORE_CONNECTIONS):
			...
			if (not MORE_CONNECTIONS_SMART):
				...
				for otherColab in collaboratorList:
					...
					nodeClass = artistDict[otherColabID]["class"]
					if (not graph.has_node(otherColabID)):
						graph.add_node(otherColabID, name=otherColabName, types=str(otherColabTypes), maxType=otherColabMaxType, node2vec_class=nodeClass)
